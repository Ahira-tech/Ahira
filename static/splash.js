/* ============================================================
   ...
   Uses AhiraContent library if available.
   
   HOW TO USE:
   1. Add <div id="ahiraSplash"></div> as FIRST child of <body>
   2. Load scripts in this order before </body>:
      <script src="/static/ahira_content.js"></script>
      <script src="/static/chat.js"></script>
      <script src="/static/splash.js"></script>
   3. Call AhiraSplash.init(2600) at top of window.onload
============================================================ */
...
const AhiraSplash = (() => {
    ...
    function getGreeting() {
        ...
        if (typeof AhiraContent !== "undefined") {
            return {
                line1: time,
                line2: AhiraContent.randomWelcome(),
            };
        }
    ...
    function buildCSS() {
        ...
        .splash-blob { position: absolute; border-radius: 50%; filter: blur(60px); pointer-events: none; z-index: 0; }
        .splash-blob-1 { width:300px;height:300px;background:radial-gradient(circle,rgba(138,108,255,0.45),transparent 70%);top:-80px;left:-60px;animation:blobFloat1 6s ease-in-out infinite; }
        .splash-blob-2 { width:250px;height:250px;background:radial-gradient(circle,rgba(244,114,182,0.35),transparent 70%);bottom:-60px;right:-40px;animation:blobFloat2 7s ease-in-out infinite; }
        .splash-blob-3 { width:180px;height:180px;background:radial-gradient(circle,rgba(251,146,60,0.2),transparent 70%);top:40%;left:60%;animation:blobFloat3 5s ease-in-out infinite; }
        ...
        .splash-ring { position:absolute;border-radius:50%;border:1.5px solid rgba(138,108,255,0.2);pointer-events:none; }
        ...
        .splash-logo-wrap{position:relative;width:88px;height:88px;margin-bottom:20px}
        ...
        .splash-logo{width:88px;height:88px;object-fit:contain;position:relative;z-index:1;filter:drop-shadow(0 0 18px rgba(138,108,255,0.7));animation:logoEntrance 0.8s cubic-bezier(0.175,0.885,0.32,1.275) 0.2s both}
        ...
        .splash-brand{font-family:'Poppins',sans-serif;font-size:32px;font-weight:800;letter-spacing:10px;background:linear-gradient(135deg,#c4a8ff 0%,#ffffff 50%,#f9a8d4 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;margin-bottom:20px;animation:brandSlideUp 0.6s ease 0.5s both}
        @keyframes brandSlideUp{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
        .splash-greeting{text-align:center;animation:greetingFade 0.6s ease 0.8s both;margin-bottom:36px;max-width:280px}
        @keyframes greetingFade{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
        .splash-line1{font-family:'Poppins',sans-serif;font-size:14px;font-weight:500;color:rgba(255,255,255,0.6);letter-spacing:0.5px;margin-bottom:6px}
        .splash-line2{font-family:'Poppins',sans-serif;font-size:17px;font-weight:700;color:rgba(255,255,255,0.95);line-height:1.4}
        .splash-particles{position:absolute;inset:0;pointer-events:none;overflow:hidden;z-index:0}
        ...
        .splash-dots{display:flex;gap:8px;animation:dotsFade 0.5s ease 1.1s both}
    ...
    function spawnParticles() {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (Math.random() > 0.45) {
                ...
                p.style.cssText = `font-size:${Math.random()*12+8}px;background:none;width:auto;height:auto;border-radius:0;`;
            } else {
                const size = Math.random() * 5 + 2;
                const color = COLORS[Math.floor(Math.random() * COLORS.length)];
                p.style.cssText = `width:${size}px;height:${size}px;background:${color};box-shadow:0 0 ${size*3}px ${color};`;
            }
